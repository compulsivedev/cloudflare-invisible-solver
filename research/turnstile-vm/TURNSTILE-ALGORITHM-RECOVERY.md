...
The XTEA key is `KJuRf8(p5[9·pad+40 : +16])` sliced from the same 128-byte buffer that the RSA block transports to the server. **Remaining boundary:** only the *dispatch* (which the `runProgram`/`V3` JSVMP closure at `decoded.js:8410` wraps) and encrypted-string telemetry events (`R["XbnH2"]`, `R["qcNv7"]`) stay in the VM; the body bytes themselves are fully reproduced in Python (see §5). `KJuRf8` (`decoded.js:2884`) is **resolved**: it is a repeating-key XOR whose 16-byte key is assembled at runtime from the string table (so it is not statically greppable), recovered live via `KJuRf8(new Uint8Array(16))` and verified against two independent `/flow/ov` captures; the port carries the captured key as `KJURF8_KEY` (`make_kjurf8`) and still exposes the transform as an injectable hook for re-probing new deployments.
...
- **The `/flow/ov` body assembly is recovered** (`pZ`, plain JS) and verified byte-for-byte; it is **not** JSVMP-driven. The residual `runProgram`/`V3` JSVMP only wraps the *dispatch* closure (and encrypted telemetry events `XbnH2`/`qcNv7`); reproducing the body bytes does not require lifting the bytecode. **`KJuRf8`** (`decoded.js:2884`), the external 16-byte transform on the XTEA key slice, is **resolved**: a repeating-key XOR whose 16-byte ASCII key is assembled at runtime from the string table (so it cannot be recovered statically), recovered live via `KJuRf8(new Uint8Array(16))` and verified against two independent live captures. The port carries the captured key as `KJURF8_KEY` (`make_kjurf8`) and keeps the injectable hook so a new deployment can be re-probed.
